...
import cloudscraper  # Usar cloudscraper en vez de requests
from dotenv import load_dotenv
import pandas as pd
import time
import numpy as np

# Crear el objeto scraper global
    ...
    for index, stringCategoria in enumerate(list_categories):
        ...
        while(existeOffset == True):
            try:
                ...
                list_products_data = scraper.get(url_page, headers=HEADERS_REQUEST_CARREFOUR)
...
def get_ids_categorys_carrefour():
    try:
        response = scraper.get(URL_CATEGORY_CARREFOUR, headers=HEADERS_REQUEST_CARREFOUR)
        # 1. Verifica si el servidor devolvió un error (403, 404, 500, etc.)
        if response.status_code != 200:
            print(f"ERROR - Carrefour respondió con código: {response.status_code}")
            if response.status_code == 403:
                print("Posible causa: La cookie es inválida o el servidor te ha bloqueado temporalmente.")
            return []

        # 2. Intenta parsear el JSON y captura errores específicos de formato
        try:
            list_category_carrefour = response.json()
        except Exception:
            print("ERROR - No se pudo procesar la respuesta de Carrefour (el JSON es inválido).")
            print(f"Respuesta parcial del servidor: {response.text[:100]}...") 
            return []

    except Exception as e:
        print(f"ERROR inesperado en Carrefour: {e}")
        return []

    # ... resto de tu código para procesar el df
    # Asegúrate de que "menu" existe en el JSON antes de usarlo
    if "menu" not in list_category_carrefour:
        print("ERROR - El JSON de Carrefour no contiene la clave 'menu'.")
        return []
    ...
    for stringCategoria in category_ids:    
        ...
        list_sub_category_carrefour_data = scraper.get(URL_SUB_CATEGORY_CARREFOUR, headers=HEADERS_REQUEST_CARREFOUR)
